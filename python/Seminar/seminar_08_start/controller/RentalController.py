...
class RentalController:
    ...
    def createRental(self, rentalId, client, car, start, end, recordUndo=True):
        ...
        '''
        if recordUndo == True:
            redo = FunctionCall(self.createRental, rentalId, client, car, start, end) 
    ...
    def deleteRental(self, rentalId, recordUndo=True):
        ...
        if recordUndo == True:
            ...
            undo = FunctionCall(self.createRental, rental.id, rental.client, rental.car, rental.start, rental.end)
            cascadeOp = CascadedOperation(Operation(redo, undo))
            self._undoController.recordOperation(cascadeOp)
        return rental
